    ...
    try:
        ...
        try:
            for lines in open(bowtievcf, 'r'):
                ...
                Depthset = [float(i) for i in lines_set[9].split('__')[-1].split(',')]
                        ...
                        if subdepth >= min_cov and subdepth / totaldepth >= min_maf_for_call:
                                ...
                                Qualified_indel, indel_result, ref_indel = compareindelallsimple(bowtievcf, allindel,
                                                                                                 [CHR, POS, REF, INDEL,
                                                                                                  subdepth, totaldepth],
                                                                                                 Qualified_indel,
                                                                                                 indel_result, ref_indel,blank_indel_set)
...
def correct_indelpos_mutated_genome(CHR,POS,allindel):
    POS = int(POS)
    for CHRref in allindel:
        if CHR == CHRref:
            for POSref in allindel[CHR]:
                if POSref < POS:
                    # indels before the current one
                    REF, INDEL = allindel[CHR][POSref]
                    if '-' in REF:
                        # NEED CHANGE LATER
                        # POS = POS - len(REF) - 1
                        indel_len = len(INDEL)
...
def compareindelallsimple(bowtievcf,allindel,indelset,Qualified_indel,indel_result,ref_indel,blank_indel_set):
    CHR, POS,REF, INDEL,subdepth,totaldepth = indelset
    POS = int(POS)
    Output = False
    InBlankIndels = correct_indelpos_mutated_genome_blank(blank_indel_set, CHR, POS, allindel)
    if CHR in allindel:
            ...
            if abs(POS - POSREF) <= 10:
                ...
                if compareindel([REF_REF, INDEL_REF], [REF, INDEL]):
                    ...
                    if [CHR,POSREF] not in ref_indel:
                        if [CHR,POSREF] not in ref_indel:
                            # if CHR == 'scaffold10|size88529': #need change
                            #     print([CHR,POSREF],CHR, POS, REF, INDEL)
                            ref_indel.append([CHR,POSREF])
                elif not InBlankIndels:
                    indel_result[1] += 1
                    Qualified_indel.append(
                        '%s\t%s\t%s\t%s\t%s\t%s\tFP_samePOS\n' % (
                            CHR, POS, REF, INDEL, subdepth, totaldepth))
    if not Output and not InBlankIndels:
        indel_result[2] += 1
        Qualified_indel.append(
...
def correct_indelpos_mutated_genome_blank(blank_indel_set,CHR,POS,allindel):
    CHR, POS = correct_indelpos_mutated_genome(CHR,POS,allindel)
    for CHRblank,POSblank in blank_indel_set:
        if CHRblank == CHR:
            if abs(POS - POSblank) <= 10:
                # indel in original assembly
                return True
    return False
        ...
        if '-' in lines:
            ...
            if totaldepth >= min_cov:
                for i in range(1, len(Depthset)):
                    ...
                    # if CHR == 'scaffold9|size114837' and POS == -36122:#need change
                    #     print(subdepth,totaldepth,subdepth / totaldepth)
    ...
    for CHR in allindel:
        ...
        for POSREF in allindelCHR:
            if [CHR, POSREF] not in ref_indel:
                ...
                indel_TP.append('%s\t%s\t%s\n' % (CHR, POSREF, '\t'.join(allindelCHR[POSREF])))
    f1 = open(mappervcf + '.TP', 'w')
    f1.write(''.join(indel_TP))
    f1.close()
    f1 = open(mappervcf + '.FN', 'w')
    f1.write(''.join(indel_FN))
    f1.close()
...
def indel_compare(allsum,samplename,blank_indel_set,set_blank_indel):
    (blank_indelbowtie, blank_indelbwa, blank_indelminimap,
        blank_indelbowtiebcfdefault, blank_indelbwabcfdefault, blank_indelminimapbcfdefault,
     blank_indelbowtiebcfdefaultindel, blank_indelbwabcfdefaultindel, blank_indelminimapbcfdefaultindel,
     blank_indelmapper,
    ...
    blank_indelbowtiemappersamtovcf, indel_result = load_indelmapper(bowtievcf, allindel, blank_indelbowtiemappersamtovcf,
                                                                     set_blank_indel)
    allsum.append('%s\tbowtie2 VARIANTS\t%s\t%s\t%s\t%s\n' % (
        samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    ...
    blank_indelbwamappersamtovcf, indel_result = load_indelmapper(bowtievcf, allindel, blank_indelbwamappersamtovcf,
                                                                  set_blank_indel)
    allsum.append('%s\tbwa VARIANTS\t%s\t%s\t%s\t%s\n' % (
        samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    # minimap mapper sam to vcf
    bowtievcf = '%s/merge/%s.minimap.mappersamtovcf.vcf.snp' % (resultdir, samplename)
    print('%s process minimap2 indel %s' % (datetime.now(), bowtievcf))
    blank_indelminimapmappersamtovcf, indel_result = load_indelmapper(bowtievcf, allindel, blank_indelminimapmappersamtovcf,
                                                                      set_blank_indel)
    allsum.append('%s\tminimap2 VARIANTS\t%s\t%s\t%s\t%s\n' % (
        samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    ...
    # blank_indelmappermappersamtovcf, indel_result = load_indelmapper(bowtievcf, allindel, blank_indelmappermappersamtovcf,
    #                                                                  set_blank_indel)
    # allsum.append('%s\tmapper VARIANTS\t%s\t%s\t%s\t%s\n' % (
    #     samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    # bowtie
    bowtievcf = '%s/merge/%s.bowtie.bcfdefault.flt.indel.vcf' % (resultdir, samplename)
    print('%s process bowtie indel %s' % (datetime.now(), bowtievcf))
    blank_indelbowtiebcfdefault, indel_result = load_indel(bowtievcf, allindel, blank_indelbowtiebcfdefault,
                                                           set_blank_indel)
    ...
    print('%s process minimap indel %s' % (datetime.now(), minimapvcf))
    blank_indelminimapbcfdefault, indel_result = load_indel(minimapvcf, allindel, blank_indelminimapbcfdefault,
                                                            set_blank_indel)
    allsum.append('%s\tminimap2 bcfdefault\t%s\t%s\t%s\t%s\n' % (
        samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    # bowtie
    bowtievcf = '%s/merge/%s.bowtie.indel.flt.indel.vcf' % (resultdir, samplename)
    print('%s process bowtie indel %s' % (datetime.now(), bowtievcf))
    blank_indelbowtiebcfdefaultindel, indel_result = load_indel(bowtievcf, allindel, blank_indelbowtiebcfdefaultindel,
                                                                set_blank_indel)
    allsum.append('%s\tbowtie2 indel\t%s\t%s\t%s\t%s\n' % (
        samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    # bwa
    bwavcf = '%s/merge/%s.bwa.indel.flt.indel.vcf' % (resultdir, samplename)
    print('%s process bwa indel %s' % (datetime.now(), bwavcf))
    blank_indelbwabcfdefaultindel, indel_result = load_indel(bwavcf, allindel, blank_indelbwabcfdefaultindel,
                                                             set_blank_indel)
    allsum.append(
        '%s\tbwa indel\t%s\t%s\t%s\t%s\n' % (
            samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    # minimap
    minimapvcf = '%s/merge/%s.minimap.indel.flt.indel.vcf' % (resultdir, samplename)
    print('%s process minimap indel %s' % (datetime.now(), minimapvcf))
    blank_indelminimapbcfdefaultindel, indel_result = load_indel(minimapvcf, allindel,
                                                                 blank_indelminimapbcfdefaultindel,
                                                                 set_blank_indel)
    allsum.append('%s\tminimap2 indel\t%s\t%s\t%s\t%s\n' % (
        samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    # # mapper
    ...
    # print('%s process mapper indel %s' % (datetime.now(), mappervcf))
    # blank_indelmappernoancestor, indel_result = load_indelmapper(mappervcf, allindel, blank_indelmappernoancestor, set_blank_indel)
    ...
    # print('%s process mapper indel %s' % (datetime.now(), mappervcf))
    # blank_indelmappernoxmer19, indel_result = load_indelmapper(mappervcf, allindel, blank_indelmappernoxmer19,
    #                                                              set_blank_indel)
    ...
    # print('%s process mapper indel %s' % (datetime.now(), mappervcf))
    # blank_indelmappernoxmer20, indel_result = load_indelmapper(mappervcf, allindel, blank_indelmappernoxmer20,
    #                                                          set_blank_indel)
    ...
    # print('%s process mapper indel %s' % (datetime.now(), mappervcf))
    # blank_indelmappernoxmer21, indel_result = load_indelmapper(mappervcf, allindel, blank_indelmappernoxmer21,
    #                                                          set_blank_indel)
    ...
    # print('%s process mapper indel %s' % (datetime.now(), mappervcf))
    # blank_indelmapperbcf, indel_result = load_indel(mappervcf, allindel, blank_indelmapperbcf,
    #                                                            set_blank_indel)
    # allsum.append('%s\tmapper (bcf)\t%s\t%s\t%s\t%s\n' % (
    #     samplename, indel_result[0], indel_result[1], indel_result[2], indel_result[3]))
    print('blank_indelminimapmappersamtovcf',blank_indelminimapmappersamtovcf)
...
for resultdir in allresultdir:
    ...
    allref = glob.glob('%s/data/%s*.0.*.snp.txt' % (resultdir, args.cluster))
    # for ref in allref:
    #     try:
    ...
    for ref in allref:
        try:
            f1 = open(ref.replace('.snp.txt', '.indel.txt'), 'r')
        except IOError:
            f1 = open(ref.replace('.snp.txt', '.indel.txt'),'w')
            print('creating 0 SNP background genome',ref.replace('.snp.txt', '.indel.txt'))
            f1.close()
    #allref = glob.glob('%s/data/%s*corrected.0.*.indel.txt'%(resultdir,args.cluster))
    allref = glob.glob('%s/data/%s*.0.*.indel.txt' % (resultdir, args.cluster))
    ...
    for ref in allref:
        ...
        allsamplename = [samplename]
        if 'ref.fasta' in samplename:
            allsamplename = glob.glob('%s/merge/*:%s.minimap.mappersamtovcf.vcf.snp' % (resultdir, samplename))
        for samplename in allsamplename:
            samplename = os.path.basename(samplename).split('.minimap.mappersamtovcf.vcf.snp')[0]
            genomename = samplename.split('.')[0]
            print(samplename,genomename)
        ...
        if '.0.SNP' not in ref:
            try:
                ...
                for samplename in allsamplename:
                    samplename = os.path.basename(samplename).split('.minimap.mappersamtovcf.vcf.snp')[0]
                    genomename = samplename.split('.')[0]
                    blank_indel_set = Ref_set.get(genomename,[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]])
                    print(genomename, [len(x) for x in blank_indel_set])
                    allsum, blank_indel_set = indel_compare(allsum, samplename, blank_indel_set, False)
            except IOError:
                pass
    f1 = open('%s/merge/modelindelsum%s.txt' % (resultdir,resultdirfilename), 'w')
